...
def load_dataset(train_size, batch_size=512, random_state=0):
    ...
    X_train, X_vald, y_train, y_vald = train_test_split(X_tmp, y_tmp, train_size=7000, random_state=random_state)
...
def compile_model(model, lr=0.01, is_lr_scheduler=True):
    ...
    lr = lr
    if is_lr_scheduler:
        # Lr(t) = init_Lr * decay_rate^(t/decay_steps)
        lr_scheduler = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=lr, decay_steps=500, decay_rate=0.95, staircase=True)
...
def train_model(model, train_ds, vald_ds, epochs=100, patience=20):
    ...
    callbacks = [tf.keras.callbacks.EarlyStopping(patience=patience, monitor='val_loss'),
                 tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path, save_best_only=True, save_weights_only=True, monitor='val_accuracy', verbose=1),
...
def create_model_for_optimization(trial, n_features, n_classes):
    ...
    r = trial.suggest_float('r', 1.0, 7.0)
    clf_num_layers = trial.suggest_int('clf_num_layers', 1, 2)
    clf_hidden_units1 = trial.suggest_int('clf_hidden_units1', 32, 160)
    ...
    # compile the model
    lr_scheduler = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=learning_rate, decay_steps=4000, decay_rate=0.95, staircase=True)
...
def main(argv):
    ...
    parser.add_argument('-d', '--data_params', nargs='*', help="Dataset parameters: batch_size train_set_percentage random_state : 512 8359 0")
    ...
    if args.data_params:
        ...
        train_size = int(args.data_params[1])
    ...
    if args.save_filename:
        ...
        path_filename = saved_model_path + 'ifeNet_heloc.h5' + filename_params + '.h5'
