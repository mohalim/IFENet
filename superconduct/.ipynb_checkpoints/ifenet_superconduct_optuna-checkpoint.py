...
def load_dataset(train_size=None, batch_size=512, is_target_norm=False, random_state=0):
    ...
    features = df.columns
    targets = 'critical_temp'

    y = df[target].values
    X = df.drop(columns=[target]).values
    
    X_tmp, X_test, y_tmp, y_test = train_test_split(X, y, train_size=13378, random_state=random_state)
    X_train, X_vald, y_train, y_vald = train_test_split(X_tmp, y_tmp, train_size=10000, random_state=random_state)
    ...
    return train_ds, vald_ds, test_ds, X_train.shape[1], C, features
...
def main(argv):
    ...
    parser.add_argument('-s', '--save_filename', type=str, help="Filename for saving the model's weights (e.g. -s ifeNet_superconduct)")
    ...
    if args.save_filename:
        ...
        path_filename = saved_model_path + 'ifeNet_superconduct' + filename_params + '.h5'
