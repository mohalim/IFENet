...
def load_dataset(train_size, batch_size=512, random_state=0):
    ...
    columns_cat = ['gender', 'SeniorCitizen', 'Partner', 'Dependents', 'PhoneService', 'MultipleLines', 'InternetService', 'OnlineSecurity',
...
def create_model_for_optimization(trial, n_features, n_classes):
    ...
    clf_hidden_size = trial.suggest_int('clf_hidden_size', 64, 82)
...
def main(argv):
    ...
    parser.add_argument('-f', '--ife_params', nargs='*', help="IFENet parameters: num_attentions r num_ife_layers clf_hidden_size (e.g. -f 8 5.6498 1 64)")
    ...
    train_ds, vald_ds, test_ds, n_features, n_classes, columns = load_dataset(train_size, batch_size, random_state)
    ...
    if args.mode == 'train':    
        ...
        study.optimize(partial_objective, n_trials=n_trials, n_jobs=-1, show_progress_bar=True)
